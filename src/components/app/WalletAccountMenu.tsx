...
export default function WalletAccountMenu({
  ...
  async function copyDepositAddress(event: MouseEvent<HTMLButtonElement>) {
    event.stopPropagation();
    if (!info?.depositAddress) return;
    await copyText(info.depositAddress);
    setCopied(true);
  }

  const registered = Boolean(info?.depositDeployed);
  const canMovePusd = (info?.ownerPusd ?? 0) > 0 && registered;
  const canMoveFromDeposit =
    (info?.withdrawableUsdc ?? 0) > 0 && registered;
