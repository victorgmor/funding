...
# DynamoDB managers (canonical profile: name / verified)
if aws dynamodb describe-table --table-name "$MANAGERS_TABLE" --region "$AWS_REGION" >/dev/null 2>&1; then
  echo "DynamoDB table $MANAGERS_TABLE already exists."
else
  echo "Creating DynamoDB table ${MANAGERS_TABLE}..."
  aws dynamodb create-table \
    ...
    --attribute-definitions AttributeName=id,AttributeType=S \
    --key-schema AttributeName=id,KeyType=HASH
  aws dynamodb wait table-exists --table-name "$MANAGERS_TABLE" --region "$AWS_REGION"
fi
...
echo ""
echo "DynamoDB tables: $FUNDS_TABLE, $MANAGERS_TABLE (FUNDS_TABLE / MANAGERS_TABLE env on ECS task)"
